...
class ActionSystem:
    ...
    def UpdateVehicles(self, final_trips, final_paths, vehicles = None):
        ...
        for vehicle, trip, path in zip(vehicles, final_trips, final_paths):
            if vehicle.path is None and len(trip.requests) == 0: # No trip
                if self.cfg.VEHICLE.REPOSITION.TYPE is True:
                    # Reposition
                    vehicle = self.reposition.Reposition(vehicle)
                    # Update itinerary nodes
                    if self.consider_itinerary:
                        vehicle.path = self.RTV_system.PlanPath.UpdateItineraryNodes(vehicle.path)
                continue
            ...
            for request in trip.requests:
                ...
                if request.pickup_position == vehicle.current_position:
                    ...
                    # Update the travel time, distance, delay
                    path.Update(self.consider_itinerary)
                else:
                    vehicle.next_requests.append(request)
            
            assert path.current_position != path.next_positions[0]
    ...
    def SimulateVehicleActions(self, vehicles = None, ConsiderIntersections = True):
        ...
        for vehicle in vehicles:
            ...
            # Update time needed to the next position
            if vehicle.remaining_time_for_current_node == 0: # The vehicle has a new path
                vehicle.time_needed_to_next_position = time_needed_to_next_position[0]
            
            vehicle.remaining_time_for_current_node += self.step_time
            for request in vehicle.current_requests:
                request.time_on_vehicle += self.step_time

            # move the vehicle to the next position
            if vehicle.remaining_time_for_current_node >= vehicle.time_needed_to_next_position:
                vehicle.remaining_time_for_current_node = 0 # Update the the remaining time for current node

                distance_interval = dis_to_next_position[0]

                # the vehicle move to the next position, so we need to check which request should be dropoff or pickup
                # Check the current requests
                new_current_requests = []
                for request in vehicle.current_requests:
                    request.distance_on_vehicle += distance_interval
                ...
                # Accumulate the distanceof the vehicle
                vehicle.total_distance += distance_interval
                ...
                if len(next_positions) > 1:
                    vehicle.time_needed_to_next_position = time_needed_to_next_position[1]
    ...
    def ProcessRequests(self):
        ...
        for request in self.requests:
            ...
            # Note: (1) Passengers may cancel their requests if the system doesn't allocate them to any vehicles for a long time (e.g., 3 mins)
